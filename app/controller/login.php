...
class login extends controller {
    ...
    function __construct() {
        parent::__construct();
        session::set('cc-title', 'Silahkan login');
        session::set('cc-log', true);

        if (session::get('cc-loggedIn') == true) {
                ...
                if ($cekID['cekID'] == true) { // jika username terdaftar
                    if ($password != '') {
                        ...
                        if ($hasil['cek'] == true) { // jika login berhasil
                            session::set('cc-username', $username);
                            session::set('cc-nama', $hasil['nama']);
                            session::set('cc-akses', $hasil['akses']);
                            session::set('cc-loggedIn', true);
                            header('location:' . URL . '/dashboard');
